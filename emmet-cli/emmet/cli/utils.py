...
def nomad_organize_data(task_ids, records, root_dir: Path, upload_preparation_dir: Path, name):
    ...
    for record in tqdm(records):
        ...
        if not full_file_path.exists():
            ...
            # get block name, launcher folder name, and launcher file name
            block_index = 0
            for p in PREFIXES:
            ...
            nomad_name = (Path(upload_preparation_dir.name) / block_name
...
def find_unuploaded_launcher_paths(outputfile, configfile, num) -> List[GDriveLog]:
    ...
    for task in task_records:
        ...
        start = 0
        for p in PREFIXES:
            if dir_name.find(p) != -1:
                start = dir_name.find(p)
